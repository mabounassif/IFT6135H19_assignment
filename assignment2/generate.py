...
parser.add_argument('--first', type=str, default='the',
                    help='first word of sequence')
...
model.load_state_dict(torch.load(args.save_dir, map_location={'cuda:0': 'cpu'}))
model.eval()

hidden = model.init_hidden()
firsts = args.first.split(',')
words_idx = torch.LongTensor([word_to_id[first] for first in firsts]).to(device)

f = open(args.model + "_len_" + str(args.seq_len) + ".txt", "w+")

for i in range(10):
    samples = model.generate(words_idx, hidden, args.seq_len)
    text = " ".join([id_2_word[sample[0].item()] for sample in samples])
    f.write(text + '\n\n')
